...
import type { Kjønn } from '../person-identifikator';
import {
  beregnKontrollsiffer1,
  beregnKontrollsiffer2,
} from './fodselsnummer-utils';
...
class FødselsnummerBeregner {
  ...
  public tilfeldigFødselsnummer() {
    ...
    const tilfeldigIndividNummer =
      gyldigeIndividNummere[
        this.faker.random.integer(gyldigeIndividNummere.length - 1)
      ];

    const justertFødselsdato = this.justerFødselsdatoForDnummer();
    const k1 = beregnKontrollsiffer1(
      justertFødselsdato + tilfeldigIndividNummer,
    );
    const k2 = beregnKontrollsiffer2(
      justertFødselsdato + tilfeldigIndividNummer + k1,
    );
  ...
  private getGyldigeIndividnummere() {
    return this.getIndividsiffereForÅrstall()
      .filter((individSiffer) =>
        this.individsifferHarRiktigKjønn(individSiffer),
      )
  ...
  private getIndividsiffereForÅrstall() {
    ...
    if (erMellom(year, 1900, 1999)) {
      return getLøpenummerSomListe(0, 500);
    }

    if (erMellom(year, 2000, 2039)) {
