...
test('Generer myndigFødselsnummer personIdentifikator', () => {
  ...
  const fødselsdato =
    navfaker.personIdentifikator.getFødselsdato(fødselsnummer);
  const alder =
    new Date(new Date().getTime() - fødselsdato.getTime()).getFullYear() - 1970;
...
test('Parser fødselsdato', () => {
  const fødselsdato =
    navfaker.personIdentifikator.getFødselsdato('10108000398');
